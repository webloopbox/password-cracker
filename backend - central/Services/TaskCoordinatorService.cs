...
namespace backend___central.Services
{
    public class TaskCoordinatorService
    {
        ...
        public async Task<bool> ProcessCompletedTasks()
        {
            ...
            catch (Exception ex) when (ex.Message.Contains("Password found!"))
            {
        ...
        {
            if (responseContent.Contains("Password found!"))
            {
                passwordFound = true;
                ILogService.LogInfo(logServices, $"Password found by server {server.IpAddress}: {responseContent}");
                CancelAllTasks();
                taskCompletionSource.TrySetException(new PasswordFoundException(responseContent));
                return true;
            }
            return false;
        }
